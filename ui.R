                ...
                sidebarLayout(
                  sidebarPanel(fluidRow(
                    ...
                    tabsetPanel(id = "dataSetupPanel", selected = "Study criteria",
                      ...
                      tabPanel("Dependence (ρ)",
                               br(),
                               sliderInput(inputId = "rhoCHE",
                                           label = p("ρ: assumed within-paper sampling correlation", style="color:#333333",
                               ...
                               bsPopover(id="q21", title = "Within-paper sampling correlation (ρ).",
                                         content = paste0("<p>When one paper contributes several effect sizes computed from the same participants ",
                                                          "(several outcome measures on one sample, repeated post-tests, or several treatment groups ",
                                                          "compared against one shared control), those effect sizes' <em>sampling errors</em> are correlated.",
                                                          "<p>This value is almost never reported by primary studies, so it is a working assumption rather ",
                                                          "than an estimate; 0.5 is the conventional choice and the default.  Vary it to check how sensitive ",
                                                          "your conclusions are to the assumption.",
                                                          "<p>It is used consistently by the aggregation, the frequentist multilevel (CHE) models, and the ",
                                                          "Bayesian multilevel models.",
                                                          "<p>Note the frequentist multilevel tabs also use cluster-robust (CR2) tests, which are designed ",
                                                          "not to lean heavily on this value.",
                                                          "<p>Changes take effect at the next press of (Re)Calculate Meta-Analysis.",
                                                          "<p>Default: 0.5"),
                                         placement = "right",
                                         trigger = "click",
                                         options = list(container = "body")),
                               p(tags$small("This is a correlation between the sampling errors of two",
                                            tags$em("different"), "effect sizes in the same paper.  It is not the",
                                            "same thing as a pre/post (test-retest) correlation, which affects how an",
                                            "individual effect size and its variance were computed before the data",
                                            "reached this app.")),
                               hr(),
                               radioButtons(inputId = "skipCHE",
                                            label = p("Escape hatch: skip the correction in the BAYESIAN multilevel models", style="color:#333333",
                                            ...
                                            choices = c(No = "No", Yes = "Yes")),
                               bsPopover(id="q22", title = "Skipping the correlated-errors correction.",
                                         content = paste0("<p><b>This is statistically the wrong model, and it is here only as an escape hatch.</b>",
                                                          "<p>Modelling the correlated sampling errors makes the Bayesian multilevel models about ",
                                                          "8 times slower to sample.  On a hosted server that can mean the fit times out or runs out ",
                                                          "of memory and never appears at all.",
                                                          "<p>Selecting Yes drops back to assuming the within-paper sampling errors are independent, ",
                                                          "which is fast.  The result will report credible intervals that are <b>too narrow</b>, and on ",
                                                          "data with shared control groups or repeated measurements the pooled estimate can shift ",
                                                          "substantially too.",
                                                          "<p>Use it to get a preliminary look when the correct model will not run here, then re-run ",
                                                          "properly on a local machine (or download the generated script) before reporting anything.",
                                                          "<p>Note that setting ρ = 0 above is <em>not</em> a substitute: it gives the same independent-errors ",
                                                          "assumption but keeps the slow computation.",
                                                          "<p>This affects only the Bayesian multilevel tabs.  The frequentist multilevel models are fast ",
                                                          "and always use the correction.",
                                                          "<p>Default: No."),
                                         placement = "right",
                                         trigger = "click",
                                         options = list(container = "body"))),
                      ...
                      tabsetPanel(id = "mainTabset",
                                  tabPanel("Explanation", 
                                           ...
                                           p('Papers often contribute several effect sizes.  The STANDARD (two-level) analyses',
                                             ...
                                             'random = ~ 1 | Paper/ID), the within-paper sampling correlation imputed at the',
                                             'ρ set in the "Dependence (ρ)" tab (default 0.5; metafor::vcalc -- the same',
                                             'assumption the aggregation makes), and cluster-robust',
                                             ...
                                             'The Bayesian multilevel model is its brms/Stan analogue, and models the same',
                                             'within-paper correlation of sampling errors (brms fcor with the same imputed ρ);',
                                             'the other Bayesian tabs are based on the bayesmeta package, which uses the',
                                             'aggregated data.'),
                                           h4("Paper:"),
                                           ...
                                           p("The source code is at",
                                             a("https://github.com/davidallbritton/living-meta-analysis-app",
                                               href="https://github.com/davidallbritton/living-meta-analysis-app", target = "_blank"),
                                             ".  To cite this version of the app, use its DOI:",
                                             a("https://doi.org/10.5281/zenodo.21938551",
                        ...
                        tabPanel("Frequentist Multilevel", value = "frequentist_multilevel",
                                 ...
                                 p('Always computed from the UNaggregated effect sizes -- every effect size',
                                   ...
                                   'within-paper sampling errors imputed as correlated at the ρ set in the',
                                   '"Dependence (ρ)" tab (default 0.5), and cluster-robust',
                        ...
                        tabPanel("Bayesian Multilevel", value = "bayesian_multilevel",
                                 ...
                                 p('Like the frequentist CHE model, this one also treats the SAMPLING errors',
                                   'of effect sizes from the same paper as correlated (brms fcor), at the ρ',
                                   'set in the "Dependence (ρ)" tab (default 0.5).  That correction is what',
                                   'makes these fits slow; it can be turned off there, at the cost of',
                                   'credible intervals that are too narrow.'),
                        ...
                        tabPanel("Bayesian Multilevel Regression", value = "bayesian_multilevel_regression",
                                 ...
                                 p('As on the "Bayesian Multilevel" tab, within-paper SAMPLING errors are',
                                   'modelled as correlated (brms fcor) at the ρ set in the "Dependence (ρ)"',
                                   'tab (default 0.5), and that correction can be turned off there at the',
                                   'cost of credible intervals that are too narrow.'),
                                 p('Always computed from the UNaggregated effect sizes, regardless of the',
