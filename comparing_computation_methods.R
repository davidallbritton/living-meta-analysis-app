...
for (i in 1:nrow(df)) {
  ...
  N <- df[i,"N_Total"]
  design <- df[i,"Design"]
  g2 <- Hedges_g(d=d, N = N, N_C=N_C, N_E=N_E, design=design)
  df[i,"g2"] <- g2
}
...
df2 <- (select(df, ID, Design, d, g, g2, N_Total, ratio) %>% 
...
metafor::escalc (measure = "SMD", m1i = mean_E, m2i = mean_C, 
                 sd1i = sd1i, sd2i = sd2i, n1i = N_Intervention, n2i = N_Control, vtype = "LS2") ->
  metafor.g.from.means

metafor::escalc (measure = "SMD", di = d,  n1i = N_Intervention, n2i = N_Control, vtype = "LS2") ->
  metafor.g.from.d

compute.es::des(d= d, n.1 = N_Intervention, n.2 = N_Control, dig = 6) -> compute.es.g.from.d
# all 3 of these produce the same value

metafor_means_g <- metafor.g.from.means$yi
metafor_means_g.var <- metafor.g.from.means$vi
